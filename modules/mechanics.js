...
const get_drawn_element = (wheel, indicator_degree) => {
    ...
    wheel.elements.forEach(element => {
        let degree_start = element.degree_start - Math.floor(element.degree_start / 360) * 360;
        let degree_end = element.degree_end - Math.floor(element.degree_end / 360) * 360;

        if(degree_end < degree_start) degree_end += 360;
    ...
    console.log('drawn element', drawn_element);
